...
backup_dir=backup.result.perts/backup.pert
cd $backup_dir
echo "a O" > command.tmp
echo "a N and HN" >> command.tmp
echo "q" >> command.tmp
cat command.tmp | make_ndx -f conf.gro -o tmp.ndx &> /dev/null
rm -f command.tmp
grompp -n index.ndx &> /dev/null
cd $cwd
...
do
    ...
    cd $backup_dir
    ln -sf $cwd/$target_dir/$ii/traj.xtc .
    ...
    rm -f hbnum.xvg
    mv -f hbnum.o.xvg hbnum.n.xvg hbnum.sol.xvg $cwd/$target_dir/$ii/
