...
%   M(\tau)
%   \Big\langle
%   O(\vect X_t) \vect b(\vect X_\tau,\tau) \vect D(\vect X_\tau)
%   \Big\rangle
% \end{align}
...
\vskip .5cm
Discussions on Eqn.~\eqref{eqn:error-o-corr}:
\begin{enumerate}
\item This equation relate the error force with the error of observable,
  which would be useful in the error studies.
\item When the system does not have long-time memory:
  \begin{align}
    p(\vect x_t,t | \vect x_{\tau}, \tau) = p(\vect x_t,t ), \qquad
    \textrm{for}\ t  - \tau > t_c
  \end{align}
  Here $t_c$ denotes the length of memory of the system. Then
  Eqn.~\eqref{eqn:error-o-corr} becomes 
  \begin{align}
    \Delta\mo (t) =
    \frac\sigma 2
    % \int \dd\vect x_0 f(\vect x_0,0)
    \int_{t - t_c}^t \dd\tau M(\tau)
    \Bigg[
    \int \dd\vect x_t\, O(\vect x_t)
    \int \dd\vect x_\tau\,
    \nabla_{\vect x_{\tau}}p(\vect x_t,t | \vect x_{\tau}, \tau)
    ...
    \bigg]
  \end{align}
  This means we can integrate \eqref{eqn:leading-rew} only from
  $t - t_c$ to $t$.
\item In our case of core set identification,
  $O(\vect x) = K_{\sigma, \vect y}(\vect x)$, which is nearly a delta function.
  A good precision of the response theory means 
  that the trajectory generated by the reference system, should
  properly sample the correlation between the
  error force flux at time $\tau$, and the probability
  density at time $t$.  In other words, we generate trajectories
  samples the conditional probability $p(\vect x_t,t | \vect x_{\tau}, \tau)$,
  and a good precision means
  the backward flux $\nabla_{\vect x_{\tau}}p(\vect x_t,t | \vect x_{\tau}, \tau)
    \cdot
    \vect D(\vect x_\tau)$ should be significant on this sampling.
\end{enumerate}
