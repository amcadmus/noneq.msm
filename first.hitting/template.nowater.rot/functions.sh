...
function make_cos_tables () {
    ...
    fht_rot_base_value=`tail -n 1 $fht_cos_base_k_file | awk '{print $2}'`
    echo $fht_rot_base_value >> base.info
}
...
function set_parameters_fht () {
    ...
    if ! `grep userreal8 $file &>/dev/null`; then
	echo "userreal8 = 0.0" >> $file
    fi
    ...
    sed -e "/^userreal8/s/=.*/= $fht_rot_base_value/g" |\
